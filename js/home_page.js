$(function(){
	...
	// 无缝滚动轮播图
	var box = $('.banner');
	var banner = $('.banber_sub');
    var nav = $('.banner_num>li');
    nav.eq(0).css('background', 'rgba(255, 136, 0, 1)');
    var w = banner.children('li').eq(0).width();
    var l = 0;
    var timer = null;
    function auto() {
        l -= w;
        ...
        nav.eq(-l / w).css('background', 'rgba(255, 136, 0, 1)').siblings().css('background', 'rgba(0, 0, 0, .5)')
    }
...
$(function(){	
	$('.nav-wrap ul li').click(function(e) {
        $('.nav-wrap ul li').addClass('select').siblings().removeClass('select')
    });
